...
class CPLUSPLUS_EXPORT FastPreprocessor: public Client
{
    ...
    void mergeEnvironment(const Path &fileName);

public:
    FastPreprocessor(const Snapshot &snapshot);

    String run(Document::Ptr newDoc, const String &source);

    // CPlusPlus::Client
    virtual void sourceNeeded(unsigned line, Path &fileName, IncludeType);

    virtual void macroAdded(const Macro &);

    virtual void passedMacroDefinitionCheck(unsigned, unsigned, const Macro &);
    virtual void failedMacroDefinitionCheck(unsigned, const StringRef &);
    ...
    virtual void startExpandingMacro(unsigned,
                                     unsigned,
                                     const Macro &,
                                     const List<MacroArgumentReference> &);
    virtual void stopExpandingMacro(unsigned, const Macro &) {}
    virtual void markAsIncludeGuard(const String &) {}
